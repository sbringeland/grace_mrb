...
ealco_e0 = importdata("ealco_e0_wf.mat");
ealco_e0(ealco_e0==-3276)=nan;
...
xind = 1:2:size(lon_map_et,1)-1;
yind = 1:2:size(lon_map_et,2)-1;
lon_map_e0 = lon_map_et(xind,yind);
lat_map_e0 = lat_map_et(xind,yind);
...
% ET
ealco_e0_sb = nan(size(ealco_e0,[1 2]));
for i = 1:size(ealco_e0,1)
    for j = 1:size(ealco_e0,2)
        ealco_lat = lat_map_e0(i,j);
        ealco_lon = lon_map_e0(i,j);
...
[row_mknz,col_mknz] = find(ealco_e0_sb==1);

% Mackenzie
for i = 1:size(row_mknz,1)
    ealco_mknz(:,i) = squeeze(ealco_e0(row_mknz(i),col_mknz(i),:));
end

ealco_e0_mknz_av = squeeze(mean(ealco_mknz,2,'omitnan'));
